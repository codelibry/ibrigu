							...
							<h5 class="cartContent__itemBody desktop-sm">
								...
								if ($attributes) {
									foreach ($attributes as $key => $attribute) {
										// Get attribute data based on product type
										if ($_product->get_type() == 'variation') {
											$attribute_label = wc_attribute_label($key);
											$attribute_value = $_product->get_attribute($key);
											$results = [$attribute_value]; // Assuming each attribute value is a string
										} else {
											$attribute_label = wc_attribute_label($attribute['name']);
											$results = wc_get_product_terms($_product->get_id(), $attribute['name'], array('fields' => 'names'));
										}
								?>
										<div class="cartContent__itemAttr">
											<?php if (!empty($attribute_label)) : ?>
												<div class="cartContent__itemAttr__label"><?php echo $attribute_label . ': '; ?></div>
											<?php endif; ?>
